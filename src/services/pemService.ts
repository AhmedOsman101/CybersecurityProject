import { modInverse } from "../lib/math.ts";
import type { PublicKey, RsaKeyComponents } from "../lib/rsa.d.ts";
import { ab2b64, base64UrlToBigInt, toBase64Url } from "../lib/utils.ts";

export class PemService {
  // Insert \n every 64 chars
  private static wrap64(b64: string): string {
  ...
  private static async exportPublicKeyPem(pub: PublicKey): Promise<string> {
    // build minimal JWK
    const jwk = {
      kty: "RSA",
      n: toBase64Url(pub.n),
    ...
    return [
      "-----BEGIN PUBLIC KEY-----",
      PemService.wrap64(b64),
      "-----END PUBLIC KEY-----",
    ].join("\n");
  }

  // Export an RSA private key to PEM (PKCS#8)
  private static async exportPrivateKeyPem(
  ...
  static async keysToPem(keys: RsaKeyComponents): Promise<{
    publicKeyPem: string;
    privateKeyPem: string;
  }> {
    const [publicKeyPem, privateKeyPem] = await Promise.all([
      PemService.exportPublicKeyPem(keys.publicKey),
      PemService.exportPrivateKeyPem(keys),
    ]);
  ...
  static async pemToComponents(
    publicKeyPem: string,
    privateKeyPem: string
  ): Promise<RsaKeyComponents> {
    const spkiBuf = PemService.pemToArrayBuffer(publicKeyPem);
    ...
    const pkcs8Buf = PemService.pemToArrayBuffer(privateKeyPem);
