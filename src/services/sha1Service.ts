...
import { rotateLeft } from "../lib/math.ts";
...
export class Sha1Service {
   ...
   */
  private static preprocess(message: string): Buffer {
  ...
  static sha1(message: string): string {
    ...
    const paddedBuf: Buffer = Sha1Service.preprocess(message);
  ...
  static async test() {
    try {
      ...
      // Encrypt
      const encrypted = Sha1Service.sha1(plaintext);
